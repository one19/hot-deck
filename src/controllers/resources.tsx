...
export const updateResources = async (
  ...
  const updated = Object.entries(value).reduce(
    (acc: ResourceState, [key, value]) => {
      if (!value) return acc;
...
export const getAllResources = async (
  ...
  const gameResources = allResources.filter((resource) =>
    resource.startsWith(gameId),
  );
...
export const deleteGameResources = async (gameId: string): Promise<void> => {
  ...
  await Promise.all(
    gameResources.map((resource) => resources.removeItem(resource)),
  );
};

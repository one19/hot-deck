...
const GradientLineShader = shaderMaterial(
  ...
  void main() {
    ...
    vec2 randomOffset = vec2(sin(time * 0.5), cos(time * 0.5)) * 0.5;
    float noise = snoise((uv + randomOffset) * 2.0 + time * 0.1);
    float contour = mod(noise * 2.0, 1.0);
    float line = step(0.95, contour); // Creates a line effect by using a step function

    vec3 colors[6] = vec3[6](vec3(0, 0, 1), vec3(0, 1, 0), vec3(1, 1, 0), vec3(1, 0.5, 0), vec3(1, 0, 0), vec3(0.5, 0, 0.5));
...
const GradientLineOverlay = () => {
  ...
  return (
    <mesh>
      ...
      <gradientLineShader ref={shaderRef} attach="material" />
    </mesh>
  );
};

export default GradientLineOverlay;
